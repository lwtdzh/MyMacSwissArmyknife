...
private let floatingPanelScreenEdgeMargin: CGFloat = 0
...
private final class EdgeConstrainedPanel: NSPanel {
    ...
    private func constrainedFrame(_ proposedFrame: NSRect) -> NSRect {
        ...
        let availableWidth = max(
            0,
            visibleFrame.width - floatingPanelScreenEdgeMargin * 2
        )
        let availableHeight = max(
            0,
            visibleFrame.height - floatingPanelScreenEdgeMargin * 2
        )
        let maximumHeight = min(
            availableHeight,
            availableWidth / Self.aspectRatio
        )
        ...
        frame.origin.x = min(
            max(
                frame.minX,
                visibleFrame.minX + floatingPanelScreenEdgeMargin
            ),
            visibleFrame.maxX - frame.width - floatingPanelScreenEdgeMargin
        )
        frame.origin.y = min(
            max(
                frame.minY,
                visibleFrame.minY + floatingPanelScreenEdgeMargin
            ),
            visibleFrame.maxY - frame.height - floatingPanelScreenEdgeMargin
        )
        return frame
    }
...
final class FloatingPanelController: NSWindowController, NSWindowDelegate {
    ...
    private func restorePlacementForCurrentConfiguration() -> Bool {
        ...
        let availableWidth = max(
            0,
            visibleFrame.width - size.width - floatingPanelScreenEdgeMargin * 2
        )
        let availableHeight = max(
            0,
            visibleFrame.height - size.height - floatingPanelScreenEdgeMargin * 2
        )
        let origin = NSPoint(
            x: visibleFrame.minX + floatingPanelScreenEdgeMargin
                + availableWidth * CGFloat(min(max(placement.relativeX, 0), 1)),
            y: visibleFrame.minY + floatingPanelScreenEdgeMargin
    ...
    private func savePlacementNow() {
        ...
        let availableWidth = max(
            0,
            visibleFrame.width - panel.frame.width - floatingPanelScreenEdgeMargin * 2
        )
        let availableHeight = max(
            0,
            visibleFrame.height - panel.frame.height - floatingPanelScreenEdgeMargin * 2
        )
        let relativeX = availableWidth > 0
            ? (
                panel.frame.minX
                    - visibleFrame.minX
                    - floatingPanelScreenEdgeMargin
            ) / availableWidth
            : 0
    ...
    private func positionAtTopRight() {
        ...
        let origin = NSPoint(
            x: visibleFrame.maxX - panel.frame.width - floatingPanelScreenEdgeMargin,
            y: visibleFrame.maxY - panel.frame.height - floatingPanelScreenEdgeMargin
        )
