...
final class AppDelegate: NSObject, NSApplicationDelegate {
    private static let mainAppBundleIdentifier = "com.mymacswissarmyknife.host"
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        floatingPanel = FloatingPanelController(
            monitor: monitor,
            settings: settings,
            openSettings: { [weak self] in self?.openMainAppSettings() }
        )
    ...
    private func createStatusItem() {
        ...
        let settingsItem = NSMenuItem(
            title: "Settings…",
            action: #selector(openMainAppSettingsAction),
            keyEquivalent: ","
        )
    ...
    @objc private func openMainAppSettingsAction() {
        openMainAppSettings()
    }
    ...
    private func openMainAppSettings() {
        ...
        let notifyMainApp = {
            notificationCenter.postNotificationName(
                Self.openResourceMonitorSettingsNotification,
                object: nil,
                userInfo: nil,
        ...
        if !NSRunningApplication.runningApplications(
            withBundleIdentifier: Self.mainAppBundleIdentifier
        ).isEmpty {
            notifyMainApp()
            return
        }
        ...
        guard let applicationURL = mainApplicationURL() else {
            NSLog("ResourceMonitor could not locate MyMacSwissArmyknife")
            return
        }
        ...
        NSWorkspace.shared.openApplication(
            ...
            if let error {
                NSLog(
                    "ResourceMonitor could not open MyMacSwissArmyknife: %@",
    ...
    private func mainApplicationURL() -> URL? {
        let embeddedApplicationURL = Bundle.main.bundleURL
            .deletingLastPathComponent()
            .deletingLastPathComponent()
            .deletingLastPathComponent()
        if Bundle(url: embeddedApplicationURL)?.bundleIdentifier
            == Self.mainAppBundleIdentifier
        {
