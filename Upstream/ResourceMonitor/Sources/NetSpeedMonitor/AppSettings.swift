...
final class AppSettings: ObservableObject {
    ...
    func setFloatingPanelAlwaysOnTop(_ enabled: Bool) {
        ...
        defaults.set(enabled, forKey: Key.floatingPanelAlwaysOnTop)
    ...
    func setDataRateUnit(_ unit: DataRateUnit) {
        dataRateUnit = unit
        defaults.set(unit.rawValue, forKey: Key.dataRateUnit)
        onPresentationChanged?()
    }
    ...
    private func savePresentation() {
        defaults.set(showMenuBar, forKey: Key.showMenuBar)
        defaults.set(showFloatingPanel, forKey: Key.showFloatingPanel)
        onPresentationChanged?()
    }

    private func saveMetricVisibility() {
        defaults.set(showDownload, forKey: Key.showDownload)
        defaults.set(showUpload, forKey: Key.showUpload)
        defaults.set(showDiskRead, forKey: Key.showDiskRead)
        defaults.set(showDiskWrite, forKey: Key.showDiskWrite)
        defaults.set(showCPU, forKey: Key.showCPU)
        defaults.set(showMemory, forKey: Key.showMemory)
        onPresentationChanged?()
    }
}
